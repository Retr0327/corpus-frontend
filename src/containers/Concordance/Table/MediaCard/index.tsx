...
function MediaCard(props: MediaCardProps) {
  const { author, board, docId, media, year } = props;
  const href = createLink(media, docId, board);

  return (
    <Group position="center">
      <HoverCard width={350} shadow="md" withArrow openDelay={200} closeDelay={400} position="top">
        <HoverCard.Target>
        ...
        <HoverCard.Dropdown>
          ...
          <List size="sm" mt="md">
            {[`board: ${getBoard(media, board)}`, `author: ${author}`, `year: ${year}`].map(
              (value, index) => (
